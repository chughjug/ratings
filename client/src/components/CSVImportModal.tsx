...
const CSVImportModal: React.FC<CSVImportModalProps> = ({ isOpen, onClose, tournamentId }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await playerApi.uploadCSV(file, tournamentId, lookupRatings);
      if (response.data.success) {
        ...
        throw new Error(response.data.error || 'Failed to upload CSV file');
      }
      setStep('preview');
    } catch (err: any) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await playerApi.importCSV(tournamentId, players, lookupRatings);
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await playerApi.downloadTemplate();
  ...
  const renderUploadStep = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600">Select a CSV file containing player data to import</p>
      </div>
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-2">
              ...
              <h2 className="text-xl font-semibold text-gray-900">Import Players from CSV</h2>
            </div>
            <button
              onClick={handleClose}
