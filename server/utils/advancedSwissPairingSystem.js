...
class AdvancedSwissPairingSystem {
  ...
  /**
   * Pair first round - highest rated vs lowest rated with alternating colors
   */
  pairFirstRound() {
    ...
    // Pair top half with bottom half using alternating colors
    for (let i = 0; i < halfCount; i++) {
      const player1 = sortedPlayers[i]; // Higher rated (top half)
      const player2 = sortedPlayers[halfCount + i]; // Lower rated (bottom half)
      
      // Alternate colors: Board 1 (higher rated white), Board 2 (lower rated white), etc.
